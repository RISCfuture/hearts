...
extension Error: LocalizedError {
  ...
  var failureReason: String? {
    switch self {
      ...
      case .couldntLoadImage:
        return String(
          localized: "Core Image couldn’t parse the image file.",
          comment: "failure reason"
        )
      case .badResponse(let response):
        if let response = response as? HTTPURLResponse {
          return String(
            localized: "Non-successful HTTP response code \(response.statusCode).",
            comment: "failure reason"
          )
        }
        return String(
          localized: "Response was not recognized as an HTTP response.",
          comment: "failure reason"
        )
      case .couldntScaleImage:
        return String(
          localized: "The resize parameter may have been invalid.",
          comment: "failure reason"
        )
      case .invalidBackgroundColor:
        return String(
  ...
  var recoverySuggestion: String? {
    switch self {
      case .invalidFilePath:
        return String(
          localized: "Verify that the image file exists at that location.",
          comment: "recovery suggestion"
        )
      case .invalidURL:
        return String(
          localized: "Make sure you are specifying the complete URL, including the scheme.",
          comment: "recovery suggestion"
        )
      case .couldntLoadImage:
        return String(
          localized: "Verify that the file is a supported image type.",
          comment: "recovery suggestion"
        )
      case .badResponse:
        return String(
          localized: "Check the URL to make sure it is working properly.",
          comment: "recovery suggestion"
        )
      case .couldntScaleImage:
        return String(
          localized: "Try using a different resize parameter.",
          comment: "recovery suggestion"
        )
      case .invalidBackgroundColor:
        return String(
