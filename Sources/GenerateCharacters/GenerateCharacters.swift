...
struct GenerateCharacters: AsyncParsableCommand {
  ...
  @Argument(
    help: "The .txt file to write to",
    completion: .file(extensions: [".txt"]),
    transform: { .init(filePath: $0) }
  )
  var output = URL(filePath: "characters.txt")

  private var sequencesURL: URL {
    .init(string: "https://unicode.org/Public/emoji/\(emojiVersion)/emoji-sequences.txt")!
  }
  private var zwjSequencesURL: URL {
    .init(string: "https://unicode.org/Public/emoji/\(emojiVersion)/emoji-zwj-sequences.txt")!
  }
  ...
  private func parseSequences(data: URLSession.AsyncBytes) async throws -> String {
    ...
    for try await line in data.lines {
      ...
      let codepointsStrings = codepointsString.trimmingCharacters(in: .whitespaces).split(
        separator: " "
      )
      let codepoints = try codepointsStrings.map {
        try parseCodepointString(String($0), allowRanges: true)
      }
  ...
  private func parseCodepointString(_ string: String, allowRanges: Bool = false) throws
    -> ClosedRange<UInt>
  {
    if allowRanges {
      ...
      switch substrings.count {
        case 1:
          return try parseCodepointString(String(substrings[0]))
        case 2:
