...
struct GenerateColors: AsyncParsableCommand {
  private static let charactersURL = Bundle.module.url(
  ...
  mutating func run() async throws {
    try await ProgressWrapper.shared.setTotal(characters.count)
  ...
  private func characterImages() async throws -> [(Character, CGImage)] {
    try await withThrowingTaskGroup(of: (Character, CGImage).self) { group in
      var images = [(Character, CGImage)]()
      try images.reserveCapacity(characters.count)
  ...
  private func averageColors() async throws -> [(Character, AverageColorData)] {
    ...
    return try await withThrowingTaskGroup(of: (Character, AverageColorData).self) { group in
      ...
      for try await entry in group where !entry.1.isEmpty {
        data.append(entry)
      }
  ...
  private func averageColors(image: CGImage) throws -> AverageColorData {
    ...
    let stdDevG = sqrt(devG / (bessel * sumA))
    let stdDevB = sqrt(devB / (bessel * sumA))
...
struct AverageColorData {
  ...
  var isEmpty: Bool {
    mean.red == 0 && mean.green == 0 && mean.blue == 0 && standardDeviation.red == 0
      && standardDeviation.green == 0 && standardDeviation.blue == 0
  }
}
